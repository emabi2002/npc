...
BEGIN
  ...
  FROM (
    CASE entity
      WHEN 'tender' THEN (SELECT reference_number FROM tenders WHERE reference_number LIKE prefix || '/' || year_part || '/%')
      WHEN 'contract' THEN (SELECT reference_number FROM contracts WHERE reference_number LIKE prefix || '/' || year_part || '/%')
      WHEN 'bid' THEN (SELECT reference_number FROM bids WHERE reference_number LIKE prefix || '/' || year_part || '/%')
    END
  ) AS refs(reference_number);
